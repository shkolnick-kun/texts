...
### [~~openSCADA~~ ~~Eclipse SCADA~~ Оно сдохло!](http://openscada.org/)
...
Клиентский SDK предназначен для встраивания в приложение и обеспечивает двухстороннюю защищенную связь с сервером Kaa в реальном времени. В составе Kaa есть генераторы SDK для C,C++,Objective C, Java.
...
Платформа для создания приложений интеренета вещей от ARM Ltd, представляет собой набор библиотек и инструментального ПО, предназначенных для создания встраиваемых систем с доступом в интернет на базе процессоров ARM. В качестве основного языка программирования выбран C++. Есть собственная RTOS на ядре RTX. Есть реализация TLS, которая, кстати, используется другими проектами, например Kaa.  Заявлена поддержка 6LoWPAN, Bluetooth Low Energy, Thread, LoRa, WiFi, NFC, RFID, Ethernet и других технологий связи. 

Есть собственное "закрытое" облако и открытый клиент для него. "Открытые" компоненты имеют различные лицензии, в т.ч. MIT, Apache и .д.

#### [RIOT](https://riot-os.org/#features)
Встраиваемая опирационня система для интернета вещей. Языки программирования C/C++. Переносима: работает на 8,16,32-разрядных микроконтроллерах. Частично совместима со стандартом POSIX, заявлена цель по обеспечению полной совместимости. Есть поддержка 6LoWPAN, IPv6, RPL, and UDP, а так же протоколов интернета вещей: CoAP, CBOR.
...
Плата для прототипирования на базе ESP8266 с открытой прошивкой. Язык программирования LUA. Асинхронный API в духе NodeJS.
Есть поддержка http (клиент и сервер), e-mail, поддержки SSL/TLS нет, безопасность обеспечивается на уровне wifi.
Прошивка лежит [тут](https://github.com/nodemcu).

### [Espruino](https://www.espruino.com/)
Открытые платы для прототипирования и прошивка к ним (написана на C). Язык программирования JS. Есть своя IDE, в том числе с визуальным созданием программ. Поддерживается довольно широкая нуменклатура аппаратного обеспечения. Есть поддержка сети и TLS (используется mbed-tls).

### [Mongoose](https://cesanta.com/)
Проект открытой OS для интернета вещей. Поддерживается несколько аппаратных платформ. Языки программирования C, JS. Поддерживается связь через: HTTP, WebSocket, MQTT, CoAP в режимах сервер, клиент, можно использовать TCP/UDP напрямую. Есть поддержка DNS: сервер, клиент. Есть поддержка TLS.

**Ахтунг!!! Двойное лицензирование!**
...
Теперь известный хипсторский язык "корпорации добра" доступен даже на AVR!!! Использует LLVM, чтобы делать нативные бинарники для микроконтроллеров.
