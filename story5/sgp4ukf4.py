...
#==============================================================================    
# Подгонка модели 
def fit_sgp4_bstar_fixed(__r, __v, __epoch, __bstar=0, g0 = 1., eps=.5, 
...
#==============================================================================
def _get_initial_estimate(r,v):
    x = np.zeros((_DIMX,),dtype=np.float)
...
def _sgp4_hx(x, **hx_args):
    ...
    argpo = x[3] + x[10] * t1
    mo    = x[4] + x[11] * t1
    n     = x[5] - x[12] * t1 - x[13] * t2 - x[14] * t3 - x[15] * t4
    return np.array((inclo, nodeo, ecco, argpo, mo, n))
#==============================================================================
class KalmanSGP4Estimator(object):
    def __init__(self, r, v, t, cb=None, m=.1, Filter=None, sp=None, R=None):
        ...
        kf = Filter(dim_x=_DIMX, dim_z=_DIMZ, dt=dt, fx=_sgp4_fx, hx=_sgp4_hx, 
...
if __name__ == '__main__':
    ...
    def kf_cb(estimator, i, j):
        pb.update(j)
        y.append(estimator.kf.y.reshape((_DIMZ,1)))
